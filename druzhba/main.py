...
from druzhba.table import ConfigurationError, TableConfig, MigrationError, InvalidSchemaError

logger = logging.getLogger('druzhba.main')


def process_database(db_alias, db_type, only_table_names, full_refresh=None, rebuild=None):
    logger.info("Beginning database %s", db_alias)
    try:
        with statsd_client.timer(f'druzhba.db.run-time.{db_alias}'):
            _process_database(db_alias, db_type, only_table_names, full_refresh, rebuild)
...
def _process_database(db_alias, db_type, only_table_names, full_refresh=None, rebuild=None):
    ...
    logger.info("%s tables ready to import for database %s",
                len(tables_yaml), db.database_alias)
    ...
    for table_yaml in tables_yaml:
        ...
        try:
            ...
            logger.info("Validated: {} / {}".format(db.database_alias, source_table_name))
        ...
        if PRINT_SQL_ONLY:
            ...
            create_statement = table.query_to_redshift_create_table(select_query, table.destination_table_name)
        ...
        while not advance_to_next_table and retries_remaining > 0:
            try:
                ...
                logger.error(str(e), table.destination_schema_name,
                ...
                advance_to_next_table = True
                    ...
                    psycopg2.extensions.TransactionRollbackError) as e:
                logger.warning(
                ...
                advance_to_next_table = True
                    ...
                    db.db_errors.DatabaseError) as e:
                retries_remaining -= 1
                if retries_remaining > 0:
                    ...
                    time.sleep((5. - retries_remaining)**2)
                else:
                    logger.error(
            ...
            logger.info("Done with %s table %s",
                        table.database_alias, table.source_table_name)
...
def run(args):
    ...
    if args.database:
        dbs = [(db['alias'], db['type'], args.tables, args.full_refresh, args.rebuild)
               for db in yaml_read if db['alias'] == args.database]
        if not dbs:
            msg = "Database {} not recognized in _databases.yml".format(
                args.database
            )
            raise ValueError(msg)
    else:
    ...
    if args.num_processes == 1:
        ...
        _ = datetime.datetime.strptime('2018-01-01 01:02:03',
                                       '%Y-%m-%d %H:%M:%S')
...
def parse_args():
    ...
    parser.add_argument(
        '-ll', '--log-level',
        help='Name of a python log level eg DEBUG'
    )
    parser.add_argument(
        '-d', '-db', '--database',
